    ...
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {[
            { label: "कुल लेख", value: articles.length, icon: "📰", color: "orange" },
            { label: "ब्रेकिंग न्यूज़", value: articles.filter(a => a.is_breaking).length, icon: "🔴", color: "red" },
            { label: "प्रकाशित", value: articles.filter(a => a.is_published).length, icon: "✅", color: "green" },
            { label: "PDF फाइलें", value: pdfs.length, icon: "📄", color: "blue" },
          ].map((stat) => (
        ...
        {tab === "articles" && (
          <div>
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={openAddForm} className="bg-orange-500 hover:bg-orange-600 text-white font-bold px-5 py-2.5 rounded-xl transition-colors flex items-center gap-2">
                ＋ नया लेख जोड़ें
              </button>
            </div>
                ...
                {articles.map((article) => (
                  <div key={article.id} className="bg-gray-900 border border-gray-800 rounded-xl p-4 flex items-center gap-4 hover:border-gray-700 transition-colors">
                    ...
                    <div className="flex gap-2 flex-shrink-0">
                      <button onClick={() => openEditForm(article)} className="bg-gray-800 hover:bg-blue-900 text-gray-300 hover:text-blue-300 px-3 py-2 rounded-lg text-sm transition-colors">
                        ✏️ संपादित
                      </button>
                      <button onClick={() => setDeleteConfirm(article.id)} className="bg-gray-800 hover:bg-red-900 text-gray-300 hover:text-red-300 px-3 py-2 rounded-lg text-sm transition-colors">
                        🗑️
                      </button>
                    </div>
                  </div>
                ...
                {pdfs.map((pdf) => (
                  <div key={pdf.id} className="bg-gray-900 border border-gray-800 rounded-xl p-4 flex items-center gap-4 hover:border-gray-700 transition-colors">
                    ...
                    <div className="flex gap-2">
                      <a href={pdf.file_url} target="_blank" rel="noopener noreferrer" className="bg-gray-800 hover:bg-green-900 text-gray-300 hover:text-green-300 px-3 py-2 rounded-lg text-sm transition-colors">
                        👁️ देखें
                      </a>
                      <button onClick={() => handleDeletePdf(pdf.id)} className="bg-gray-800 hover:bg-red-900 text-gray-300 hover:text-red-300 px-3 py-2 rounded-lg text-sm transition-colors">
                        🗑️
                      </button>
                    </div>
                  </div>
          ...
          <div className="bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-2xl my-8 shadow-2xl">
            ...
            <form onSubmit={handleFormSubmit} className="p-6 space-y-5">
              ...
              <div>
                ...
                <input
                  type="text" value={form.title_hi} onChange={e => setForm(f => ({ ...f, title_hi: e.target.value }))}
              ...
              <div>
                ...
                <input
                  type="text" value={form.title_en} onChange={e => setForm(f => ({ ...f, title_en: e.target.value }))}
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4">
          <div className="bg-gray-900 border border-gray-700 rounded-2xl p-8 max-w-sm w-full text-center shadow-2xl">
            ...
            <div className="flex gap-3">
              <button onClick={() => setDeleteConfirm(null)} className="flex-1 bg-gray-800 hover:bg-gray-700 text-gray-300 font-medium py-3 rounded-xl transition-colors">
                रद्द करें
              </button>
              <button onClick={() => handleDelete(deleteConfirm)} className="flex-1 bg-red-600 hover:bg-red-700 text-white font-bold py-3 rounded-xl transition-colors">
                हाँ, हटाएं
              </button>
            </div>
          </div>
