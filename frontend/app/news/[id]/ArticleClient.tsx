...
function YouTubeEmbed({ videoId }: { videoId: string }) {
  return (
    <div className="my-4 sm:my-6 rounded-xl overflow-hidden shadow-lg" style={{ aspectRatio: '16/9' }}>
  ...
  return (
    <div className="flex items-center gap-1.5 sm:gap-2 bg-gray-100 dark:bg-gray-800 rounded-full px-2.5 sm:px-3 py-1 sm:py-1.5">
      ...
      <button onClick={() => setSize(Math.max(14, size - 2))}
        className="w-5 h-5 sm:w-6 sm:h-6 rounded-full bg-white dark:bg-gray-700 shadow text-gray-600 dark:text-gray-300 hover:bg-orange-50 transition-colors text-xs sm:text-sm font-bold flex items-center justify-center">−</button>
      <button onClick={() => setSize(Math.min(26, size + 2))}
        className="w-5 h-5 sm:w-6 sm:h-6 rounded-full bg-white dark:bg-gray-700 shadow text-gray-600 dark:text-gray-300 hover:bg-orange-50 transition-colors text-xs sm:text-sm font-bold flex items-center justify-center">+</button>
      <span className="text-base sm:text-lg text-gray-500 dark:text-gray-400 font-bold">अ</span>
    </div>
  );
}
