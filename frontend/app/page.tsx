...
export default function Home() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/comments/counts`).then(r => r.json()),
    ]).then(([newsRes, breakingRes, countsRes]) => {
      setArticles(newsRes.data || []);
      if (breakingRes.data?.length > 0)
        setBreakingNews(breakingRes.data.map((n: Article) => n.title_hi).join('   •   '));
      setCommentCounts(countsRes.data || {});
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                <div>
                  <h1 style={{
                    ...
                    fontWeight: 900, lineHeight: '1.4', paddingTop: '3px', paddingBottom: '3px',
                    background: 'linear-gradient(to right,#f97316,#ef4444)',
                    WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent',
                    backgroundClip: 'text', whiteSpace: 'nowrap',
                    fontSize: 'clamp(15px,3vw,20px)', display: 'block',
                  }}>
          ...
          {!loading && filtered.length === 0 && (
            <div className="flex flex-col items-center justify-center py-32 gap-3">
              ...
              <button onClick={() => { setSearchQuery(''); changeCategory('सभी'); }}
                className="mt-2 text-orange-500 font-semibold text-sm underline">सभी खबरें देखें</button>
            </div>
          )}
              ...
              <div className="xl:col-span-3 space-y-8">
                ...
                {hero && (
                  <div className="fade-up">
                    <Link href={`/news/${hero.id}`} className="block"><HeroCard article={hero} /></Link>
                  </div>
                )}
                  ...
                  {grid.map((article, i) => (
                    <div key={article.id} className="fade-up" style={{ animationDelay: `${i * 50}ms` }}>
                      <Link href={`/news/${article.id}`} className="block">
                        <GridCard article={article} commentCounts={commentCounts} />
                      </Link>
                    </div>
                  ))}
                </div>
